...
# INFARGATE is set in the task def
if "true" in os.getenv("INFARGATE", "false"):
    region_endpoint = os.getenv("ECS_CONTAINER_METADATA_URI_V4")
    url = f"{region_endpoint}/task"
    print("Using fargate endpoint: ", url)
    
    try:
        metadata = requests.get(url).json()
    except requests.exceptions.HTTPError as err:
        region = None
        print("AWS metadata endpoint not found ", err)
        pass
    ...
    try:
        # extract region from cluster ARN as they don't currently return AZ or region
        match = re.search(r"^(?:[^:]+:){3}([^:]+).*", metadata['Cluster'])
        ...
        ssm = None
        print("AWS Region not found, probably running locally")
        pass
else:
    print("Running locally")
    ssm = None

def get_param(arg1, arg2):
    # if we supply an env var for this but not running in AWS, look up the value of arg1 and return it
    if ssm is None:
        param = os.getenv(arg1)
        if param is not None:
            return param
        else:
            return arg2

    # otherwise, look up the param and return the value
    env = os.getenv("ENVIRONMENT")
    # try to get the param, if not found for whatever reason, use the second value
    try:
        print(f"Looking for /{env}/{arg1}")
        value = ssm.get_parameter(Name=f"/{env}/{arg1}", WithDecryption=True)
...
class ServerConfig(object):
    SECRET_KEY: str = get_param('SECRET_KEY', empty_str_cast(config_ini["server"]["SECRET_KEY"]) or gen_secret_key())

    DATABASE_URL: str = get_param('DATABASE_URL', f"sqlite:///{os.path.dirname(os.path.abspath(__file__))}/ctfd.db")
    # DATABASE_URL: str = empty_str_cast(config_ini["server"]["DATABASE_URL"]) or f"sqlite:///{os.path.dirname(os.path.abspath(__file__))}/ctfd.db"

    REDIS_URL: str = get_param('REDIS_URL', None)
    ...
    if UPLOAD_PROVIDER == "s3":
        ...
        AWS_S3_BUCKET: str = get_param('AWS_S3_BUCKET', empty_str_cast(config_ini["uploads"]["AWS_S3_BUCKET"]))
